<?php 
	...
	if (isset($_GET['subject_code'])) {
		...
		if ($rows > 0) {
			...
			$message = "<p class='error'>Subject not found.</p>";
		}
	} else {
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST") {
		...
		if ($rows == 1) {
			$sql = "UPDATE tbl_subject SET subject_code = '$subject_code', subject_name = '$subject_name', department_code = '$department_code' WHERE subject_code = '$subject_code'";
	...
	<div class="container">
		...
		<form method="post" action="">
			<div class="input-group">
				...
				<input type="text" name="subject_code" id="subject_code" placeholder="Subject Code" required value="<?php echo isset($data['subject_code']) ? $data['subject_code'] : ''; ?>">
			</div>
			<div class="input-group">
				<label for="subject_name">Subject Name</label>
				<input type="text" name="subject_name" id="subject_name" placeholder="Subject Name" required value="<?php echo isset($data['subject_name']) ? $data['subject_name'] : ''; ?>">
			</div>
			<div class="input-group">
